...
para.iter <- res1;
QCindex = as.numeric(res1[1]);
lam <- as.numeric(res1[2]);
...
bhat = se = tstatres = matrix(0, ncol = 2, nrow = length(PVA));
...
for(irep in 1:length(PVA)){
  ...
  colnames(bhat) = colnames(se) = c("MRLD", "MRLDP");
  save(PVA, pres, MHCpres, Xpres, Ypres, bhat, se, tstatres, file = filename);
